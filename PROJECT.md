...
### 3.2 บทบาทครบ 30 บท (`ROLES` ใน `app.js`)
...
- ตรวจแล้ว: `npm run check` + `npm test` **141/141 ผ่าน** (audit v2.5.0 → กฎโหวต v2.5.1 → ปกการ์ดแจก/ซ่อนการ์ดธีม v2.5.2 → shield ปกการ์ดเท่ากันเป๊ะ v2.5.3 → S18 ครบทุกบทบาท 30/30 → audit รอบ 2 กลไก/ความปลอดภัย/UX/PWA v2.5.4)
...
- ✅ **Test อัตโนมัติ (E2E)** — `test/smoke.mjs` (Node + Chrome DevTools Protocol, **141 checks**, test เองไม่มี dependency) รันด้วย `npm test` — _ทำเสร็จในรอบ P1 + ขยาย S16/S17/S18 + audit รอบ 2_
...
28. ✅ **ปกการ์ดแจกบทบาทเป็นภาพ + ซ่อนการ์ด "ธีมและตัวเลือก"** — การ์ดปิด (แตะเพื่อดู) แสดงภาพ **`assets/role.jpg` (หลังการ์ด tarot) 180×240** ผ่าน **`.rv-shield` (absolute inset:0 padding เท่า `.rv`) ทับ `.rv-blind` (`visibility:hidden` — เนื้อหาจริงค้ำความสูง)** → การ์ดปิด/เปิด **ความสูงเท่ากัน 100% + ตำแหน่งภาพแนวตั้ง/แนวนอนตรงกันเป๊ะ** (ทั้งคู่ `flex-start` + `padding-top:36px`, `.rv-body` width:100% ให้ความกว้าง `.ri` เท่ากัน) · **ลบการ์ด "ธีมและตัวเลือก" ออกจากหน้าตั้งค่า** — ค่าที่ตั้งไว้ (ธีม/ขนาดตัวอักษร/ค้างจอ/สั่น/เสียง) ยังคงมีผลและเปลี่ยนผ่าน `setTheme`/`setFont`/`toggleSetup` ได้เหมือนเดิม · test S11 ปรับเป็นตรวจ "การ์ดถูกซ่อน + ค่ายังใช้ได้" + check ปรับเป็น `.rv-shield .rimg` + วัด `closedH` เทียบ `openH` (±2px) → **E2E 108/108 ผ่าน** · `sw.js` bump **v2.5.3**
29. ✅ **ตรวจกลไกครบทุกบทบาท 30/30 (S18)** — เขียน test ใหม่ `S18` ไล่ assert กลไก "จริง" ของ **ทุกบทบาททั้ง 30 ใบ** — ปิดจุดที่ก่อนหน้าหลายบทบาทแค่มี "ชื่อ" โผล่ใน test แต่ไม่เคยเช็คกลไก: `apprentice_seer` เลื่อนขั้นตรวจแทน, `wolfcub` ตาย → คืนถัดไปฆ่า 2, `mayor` เสียงโหวต x2 (`tally`), `cursed` ถูกกัดกลายร่าง (+ การ์ดหมาป่าจากคำสาป), `virginia_woolf` ตาย → คนที่เลือกตายตาม (`fear`), `cupid` จับคู่ + คู่รักต่างฝ่ายเหลือ 2 คนชนะ (`lovers`), `prince` โหวตครั้งแรกไม่ตาย/ครั้งสองตาย, `infected` ถูกกัด → หมาป่าติดเชื้อฆ่าไม่ตาย, `witch` ยารักษา+ยาพิษ, `bodyguard`/`priest` (ใช้ครั้งเดียวแล้วหายจากกลางคืน)/`pi` (ตรวจนัดเดียวเจอหมาป่า+ใช้แล้วหาย), `sorceress` ค้นเจอเทพจริง, `minion` เห็นชื่อหมาป่าในการ์ด, `hunter` ยิงโต้, `tanner`/`cult_leader`/`hoodlum`/`lone_wolf` เงื่อนไขชนะ ฯลฯ → **E2E 140/140 ผ่าน — ไม่พบบั๊กในบทบาทใดเลย** (test file + docs เท่านั้น ไม่แตะ shell → ไม่ bump sw)
30. ✅ **ตรวจบั๊กทั้งโปรเจครอบ 2 (audit 4 มิติ: กลไก+state / ความปลอดภัย / UX-CSS / PWA-docs)** — **กลไก/State**: `confirmTroublemaker` ตั้ง `forceVoteRound = S.g.round` (เดิม `round+1` ผิด off-by-one → จุดเช็ค `=== S.g.round` ทุกจุดทำงานทันวันเดียวกัน) · **ลูกหมาป่าโบนัส** เปลี่ยนเป็น flag `wolfCubBonusUsed` (เดิม `!night.wolfCubBonusActive` สลับ true/false ตลอดกาล → โบนัสได้แค่ 1 คืน) · `startGame`/`newGameEnd` **เคลียร์ `CORRUPT_BLOCK_KEY`** (เดิมบล็อกค้าง → เกมใหม่ในช่องที่เคยเสียหายบันทึกไม่ได้) · `fromExecution` เช็ค **tanner ใน `exDeaths`** (แขวนยาจกผ่านคู่รักชนะทันที เหมือน `fromDawn`) · `selectVoter` บล็อกคน **ปิดปาก** + `finishVoting` วันบังคับยกเว้น silenced (กันเกมค้าง) · `startT` reset 300 วิเมื่อ timer ≤ 0 · `requestWakeLock` guard/identity กันซ้ำ · **ความปลอดภัย**: history `esc(wLabel)` + `Number(h.n)/(h.round)`, preset `esc(JSON.stringify(p.id))`, `.assetsignore` เพิ่ม `*.md` (PROJECT/README/TESTING ไม่ถูกเสิร์ฟ), `.gitignore` เพิ่ม `.env`/`.dev.vars` · **UX/CSS**: `.sheet.dlg` overflow + `.dlg-msg` เลื่อนได้ (`ov-open` add/cleanup), `--muted2` contrast AA (dark `#7b8199`/light `#646a80`), `body`/`.ux-topbar`/`#modTrig` safe-area inset ทั้งหมด, touch target 44px (`.ux-back`/`.sheet-head .x`), `.btn role=button tabindex=0` + global keydown Enter/Space, `askHunterShoot` confirm wrapper, `balanceWarnings` `||0` + ข้อความ "เริ่มเกมไม่ได้", chip opacity dis/done/skip · **PWA/Infra**: `sw.js` bump **v2.5.4** + `safeMatch` จำกัด `caches.open(CACHE)` (เดิม global `caches.match` อาจข้าม namespace), package.json v**12.0.0** + `wrangler` devDep, smoke hardening (SIGINT/SIGTERM → cleanup, non-Windows `pkill -f ww-smoke-`, `CHROME_PATH` override, `startServer` reject on error, `main().catch`) · **Test**: S18 troublemaker assert `=== S.g.round` + `isForceVoteDay`, wolfcub regression `noRepeat3/noRepeat4`, S14 ปิดปาก+วันบังคับ, S15 เช็ครูปบทบาท ⊆ `CORE_ASSETS`, driver `collectVotes` กรอง silenced → **E2E 141/141 ผ่าน** · `sw.js` bump **v2.5.4** · _deferred (ผู้ใช้สั่งไว้)_: text-size em scale / CSP / multi-tab sync / pushState back gesture / dialog focus trap / controllerchange reload
...
| บทบาทครบ 30 + เอฟเฟกต์ครบ                                                                                                                                                                                                                                                               | ✅ ครบ                                    |
| Undo / History / Save-Resume                                                                                                                                                                                                                                                            | ✅ มี                                     |
| ธีม, จับเวลา, Wake Lock, Haptic                                                                                                                                                                                                                                                         | ✅ มี                                     |
| **P0: Git / archive / PWA / checklist**                                                                                                                                                                                                                                                 | ✅ **เสร็จแล้ว**                          |
| Smoke test จริงครบทุกข้อใน `TESTING.md`                                                                                                                                                                                                                                                 | ⬜ ยังไม่ได้ทำ (แต่มี E2E 141/141 แทน)    |
